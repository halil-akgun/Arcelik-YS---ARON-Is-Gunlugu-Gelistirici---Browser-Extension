...
// INIT
async function init() {
    console.log("[ARON] editor açıldı");

    const tableData = await getTableData();

    console.log("[ARON] alınan data:", tableData);

    if (!tableData.length) {
        document.body.innerHTML = "<h3>Veri bulunamadı</h3>";
        return;
    }

    table = createTableFromData(tableData);
...
async function editPage() {
    ...
    dataRows.forEach((row) => {
        ...
        if (tds.length >= 12) {
            ...
            // 12. hücreye dropdown (seçim kutusu) ekle
    ...
    totalExpenseRow.id = "totalExpenseRow";
